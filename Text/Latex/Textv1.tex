...
\begin{figure}[h]
\caption{2-pole High filter frequency response}
\centering
\resizebox{\textwidth}{!}{
...
\addplot[domain=1:1e4,color=red] {x*x/(x*x + 500*x + 250000)};
...
It does have it's flaws though, mainly that phase is not kept constant throughout the transform, and this means it is possible that the signal oversteps the -1/1 bound. Being a feedback loop, this error then enters every other value and corrupts every future signal, even future frames.
To limit this, I simply placed a max() bound at the appropriate place, preventing at least any distortion from spreading.
