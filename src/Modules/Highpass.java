...
public class Highpass extends Module
{
	...
	public Highpass(Container container)
	{
		super(container);
		...
		input_pipe_names[SOUND_INPUT] = "Sound signal to be highpassed";
		...
		for (int side=0; side<audio_mode; side++)
		{
			...
			for (int f=0; f<Constants.SNAPSHOT_SIZE; f++)
			{
				...
				else
				{
					...
					factor = Math.pow((f-cutoff), 2)/(Math.pow((f-cutoff), 2) + (f-cutoff)/Q + 1);
				}
